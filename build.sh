...
function AnyKernel3() {
	END=$(date +"%s")
	DIFF=$((END - START))
	VERSION="Redline-${DEVICE}-${DATE}"
	zipname="$VERSION.zip"
	if [ -f "out/arch/arm64/boot/Image" ] \
	&& [ -f "out/arch/arm64/boot/dtbo.img" ] \
	&& [ -f "out/arch/arm64/boot/dtb" ]; then
		if [ "${DEVICE}" = "alioth" ]; then
			git clone -q https://github.com/madmax7896/AnyKernel3.git -b alioth --depth=1
		elif [ "${DEVICE}" = "apollo" ]; then
			git clone -q https://github.com/madmax7896/AnyKernel3.git -b apollo --depth=1
		elif [ "${DEVICE}" = "lmi" ]; then
			git clone -q https://github.com/madmax7896/AnyKernel3.git -b lmi --depth=1
		elif [ "${DEVICE}" = "munch" ]; then
			git clone -q https://github.com/madmax7896/AnyKernel3.git -b munch-uvite --depth=1
		else
			git clone -q https://github.com/madmax7896/AnyKernel3.git -b psyche --depth=1
		fi
